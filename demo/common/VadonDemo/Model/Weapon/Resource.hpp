...
{
	struct WeaponDefinition : public Vadon::Model::Resource
	{
		...
		Vadon::Model::SceneID projectile_prefab;
		float rate_of_fire = 1.0f;

		static void register_resource(::Vadon::Foundation::TypeMetadataRegistry& metadata_registry);
	};

	VADON_MODEL_DECLARE_TYPED_RESOURCE_ID(WeaponDefinition, WeaponDefID);
	VADON_MODEL_DECLARE_TYPED_RESOURCE_HANDLE(WeaponDefinition, WeaponDefHandle);
}
