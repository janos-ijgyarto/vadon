// NOTE: taken from the QTFileCustomBuild sample in Sharpmake
using Sharpmake;
using System;
...
namespace Vadon.Utilities
{
    ...
    public class QtSharpmakeMocTool
    {
        ...
        public QtSharpmakeMocTool()
        {
            ...
            AdditionalDefines.Add(new AdditionalDefinition(Sharpmake.Platform.win64 | Sharpmake.Platform.win32, Sharpmake.DevEnv.vs2022, "WIN32", "_MSC_VER=1944"));
        }
        ...
        {
            public RccSourceAndTargetFile(string targetName, string rccExe, string outputFolder, string sourceFile, string sourceRoot)
            {
                // NOTE: mirroring the folder structure of the files that the generated files are based on
                string relativePath = Path.GetDirectoryName(Path.GetRelativePath(sourceRoot, sourceFile)) + @"\";
                string fileOutputFolder = Path.Combine(outputFolder, relativePath);

                Executable = rccExe;
                KeyInput = sourceFile;
                string ResourceName = Path.GetFileNameWithoutExtension(sourceFile);
                Output = fileOutputFolder + "qrc_" + ResourceName + ".cpp";
        ...
        {
            public UicSourceAndTargetFile(string targetName, string uicExe, string outputFolder, string sourceFile, string sourceRoot)
            {
                // NOTE: mirroring the folder structure of the files that the generated files are based on
                string relativePath = Path.GetDirectoryName(Path.GetRelativePath(sourceRoot, sourceFile)) + @"\";
                string fileOutputFolder = Path.Combine(outputFolder, relativePath);
                ...
                Executable = uicExe;
                KeyInput = sourceFile;
                Output = fileOutputFolder + "ui_" + Path.GetFileNameWithoutExtension(sourceFile) + ".h";
        ...
        public void GenerateListOfFilesToMoc(Project project, string QTExecFolder)
        {
            ...
            foreach (ProjConfiguration conf in project.Configurations)
            {
                ...
                {
                    // NOTE: mirroring the folder structure of the files that the generated files are based on
                    string relativePath = Path.GetDirectoryName(Path.GetRelativePath(project.SourceRootPath, file)) + @"\";
                    string fileOutputFolder = Path.Combine(outputFolder, relativePath);

                    var target = new MocSourceAndTargetFile(targetName, mocExe, fileOutputFolder, fileOutputFolder, file);
                ...
                if (qrcFiles.Count > 0)
                {
                    RccTargetsPerConfiguration.Add(conf, qrcFiles.Select(file => new RccSourceAndTargetFile(targetName, rccExe, outputFolder, file, project.SourceRootPath)).ToList());
                }
                if (uiFiles.Count > 0)
                {
                    UicTargetsPerConfiguration.Add(conf, uiFiles.Select(file => new UicSourceAndTargetFile(targetName, uicExe, outputFolder, file, project.SourceRootPath)).ToList());
                }
            }
